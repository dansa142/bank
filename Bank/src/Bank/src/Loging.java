...
public class Loging {
	...
	public static void recored(Client client, int x, double amount) throws IOException {
		...
		switch (x) {
		case 0:
			bufferedWriter.write(dateFormat.format(today) + "- Withdrawl amount: " + amount + "Ballance: "
					+ String.format("%12.2f", client.getBallance()));
			bufferedWriter.newLine();
			bufferedWriter.close();
			break;

		case 1:
			bufferedWriter.write(dateFormat.format(today) + "- Deposite amount: " + amount + "Ballance: "
					+ String.format("%12.2f", client.getBallance()));
			bufferedWriter.newLine();
			bufferedWriter.close();
			break;
		...
		case 2:
			bufferedWriter.write(dateFormat.format(today) + "- Quarterly Intrest fees for ballance minus: "
					+ String.format("%15.2f", amount) + " Updated ballance: "
					+ String.format("%12.2f", client.getBallance()));
			bufferedWriter.newLine();
			bufferedWriter.close();
			break;
